...
public class QieHuanLoginActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ActivitiyUtils.setActionBarLayout(getActionBar(), this,
				R.layout.qiehuantitle_bar_login);
		initView();
	}
	...
	private void login(String username, String password) {
		...
		http.send(HttpMethod.POST, url, params, new RequestCallBack<String>() {
			...
			public void onFailure(HttpException arg0, String arg1) {
				Toast.makeText(QieHuanLoginActivity.this, "该用户不存在！",
			...
			public void onSuccess(ResponseInfo<String> responseInfo) {
				...
				if (list.size() == 1) {
					MyApplication.saveLoginStatus("login",
							list.get(0).getUid(), 
							list.get(0).getUphone(), 
							list.get(0).getUpassword(),
							list.get(0).getUprovince()+list.get(0).getUcity()+
						    list.get(0).getUcountry()+list.get(0).getUdetail());

				
					Intent intent = new Intent(QieHuanLoginActivity.this,
							MainActivity.class);
					/*intent.putExtra("flag", "1");
					intent.putExtra("uid", list.get(0).getUid()+"");*/
					MyApplication.myflag="1";
					MyApplication.SP_user_id=list.get(0).getUid();
					
					
					startActivity(intent);

				} else {
					Toast.makeText(QieHuanLoginActivity.this, "fail",
							Toast.LENGTH_SHORT).show();
				}
